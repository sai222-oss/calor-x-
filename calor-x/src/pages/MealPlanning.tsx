...
const MealPlanning = () => {
    ...
    if (!planLoading && !isPro) {
        return (
            <div className="min-h-screen" style={{ background: "#F9F9F2" }}>
                <div className="p-4 text-white flex items-center gap-3" style={{ background: "#1B4332" }}>
                ...
                <div className="flex flex-col items-center justify-center p-8 text-center space-y-6 pt-16">
                    ...
                    <div>
                        <h2 className="text-2xl font-black mb-2" style={{ color: "#1B4332" }}>{t("upgrade_title")}</h2>
                    ...
                    <Card className="w-full max-w-xs p-5 text-left space-y-3 premium-card">
                        {["🍽️ خطة وجبات أسبوعية", "🤖 اقتراحات AI مخصصة", "📖 وصفات عربية صحية", "💪 تغذية مبنية على أهدافك"].map((item) => (
                            <div key={item} className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-medium" style={{ color: "#1B4332" }}>{item.slice(3)}</span>
                            </div>
                        ))}
                    </Card>
                    <Button className="w-full max-w-xs py-6 rounded-2xl font-bold text-lg btn-glow" style={{ background: "#1B4332" }} onClick={() => navigate("/pricing")}>
    ...
    return (
        <div className="min-h-screen pb-24" style={{ background: "#F9F9F2" }}>
            ...
            {/* Tabs */}
            <div className="flex gap-1 p-3 sticky top-0 z-10" style={{ background: "#F9F9F2", borderBottom: "1px solid rgba(27, 67, 50, 0.1)" }}>
                    ...
                    <div className="space-y-4">
                        <Card className="p-5 premium-card">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h2 className="font-bold" style={{ color: "#1B4332" }}>{t("meal_plan_ai_title")}</h2>
                            </div>
                            ...
                            {aiSuggestion ? (
                                <div className="bg-white/50 rounded-2xl p-4 text-sm leading-relaxed whitespace-pre-wrap" style={{ color: "#1B4332" }}>
                                    {aiSuggestion}
                                </div>
                            ) : null}
                            <Button
                ...
                {activeTab === "week" && (
                    <div className="space-y-3">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <h2 className="font-bold text-sm" style={{ color: "#1B4332" }}>{t("meal_plan_week_title")}</h2>
                        </div>
                        {WEEKLY_PLAN.map((day, i) => (
                            <Card key={i} className="p-4 premium-card">
                                <h3 className="font-bold mb-3 pb-2 border-b" style={{ color: "#1B4332", borderColor: "rgba(27, 67, 50, 0.1)" }}>
                                ...
                                <div className="space-y-2">
                                    {(["breakfast", "lunch", "dinner"] as const).map((mealKey) => {
                                        ...
                                        return (
                                            <div key={mealKey} className="flex items-start justify-between gap-2 p-2.5 rounded-xl" style={{ background: "#F9F9F2" }}>
                                                <div className="flex-1">
                                                    <span className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">{mealLabel}</span>
                                                    <p className="text-sm font-medium mt-0.5" style={{ color: "#1B4332" }}>
                                                        {lang === "ar" ? meal.ar : meal.en}
                                                    </p>
                                                </div>
                                                <div className="text-right shrink-0">
                                                    <p className="text-xs font-bold" style={{ color: "#D4AF37" }}>{meal.cal} {t("meal_plan_recipe_cal")}</p>
                            ...
                            {RECIPES.map((recipe, i) => (
                                <Card key={i} className="p-4 premium-card">
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            <h3 className="font-bold text-base mb-1" style={{ color: "#1B4332" }}>
                                        ...
                                        <div className="text-right ml-4 shrink-0">
                                            <p className="text-xl font-black" style={{ color: "#D4AF37" }}>{recipe.cal}</p>
                                            <p className="text-[10px] text-muted-foreground">{t("meal_plan_recipe_cal")}</p>
                                            <p className="text-sm font-bold mt-1" style={{ color: "#1B4332" }}>{recipe.protein}g</p>
